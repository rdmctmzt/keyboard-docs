...
| **先读后写** | `setFuncInfo` / `patchFuncInfo` 只改传入字段；其余字节保持上次 `getFuncInfo` 读到的原始值，不会把开关 `0` 重写成 `1` |
...
用上次读到的功能区做底，只覆盖对象里相对上次发生变化的字段（CMD `0x15`）。没读过会先读。未改动的字节（包括值为 `0` 的开关）原样写回。

### 参数

| 参数 | 类型 | 说明 |
|---|---|---|
| `data` | `FuncInfo` | 要写入的字段。与上次读到的值相同的字段不会改设备字节 |
...
1. **只改点名字段**：写入以上次 `getFuncInfo` 的原始字节为底，没出现在本次修改里的字节原样发回。不要自己拼一整包，否则点阵屏开关读到 `0`（开）会被重写成 `1`（关）。  
